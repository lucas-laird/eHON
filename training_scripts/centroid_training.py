...
def train_epochs(model, train_loader, test_loader, optimizer, num_epochs = 10, add_train = None, add_test = None, scheduler = None, log_iter = 10, device = 'cuda:0', log_dir = "eHON_checkpoints/"):
...
if __name__ == '__main__':
    ...
    test_loader = DataLoader(test_dataset, batch_size = 4, num_workers = 4, follow_batch = follow_batch)
    if use_add:
        add_train_loader = DataLoader(add_train, batch_size = 8, num_workers = 4, follow_batch = follow_batch)
        add_test_loader = DataLoader(add_test, batch_size = 4, num_workers = 4, follow_batch = follow_batch)
    else:
